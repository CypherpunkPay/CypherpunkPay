...
from cypherpunkpay.ln.lnd.lnd_client import LndClient, InvalidMacaroonLightningException
...
class LndClientTest(CypherpunkpayNetworkTestCase):
    ...
    def test_malformed_lnd_url(self):
        with pytest.raises(LightningException):
            LndClient('malformed url', lnd_invoice_macaroon=self.DEV_LND_INVOICE_MACAROON, http_client=self.clear_http_client).create_invoice()

    def test_lnd_down(self):
        with pytest.raises(LightningException):
            LndClient('https://127.0.0.1:65535', lnd_invoice_macaroon=self.DEV_LND_INVOICE_MACAROON, http_client=self.clear_http_client).create_invoice()

    def test_cant_be_http(self):
        with pytest.raises(LightningException):
            LndClient('http://127.0.0.1:8081', lnd_invoice_macaroon=self.DEV_LND_INVOICE_MACAROON, http_client=self.clear_http_client).create_invoice()

    def test_malformed_lnd_invoice_macaroon(self):
        with pytest.raises(LightningException):
            LndClient(self.DEV_LND_URL, lnd_invoice_macaroon='0ea3', http_client=self.clear_http_client).create_invoice()

    def test_invalid_lnd_invoice_macaroon(self):
        with pytest.raises(InvalidMacaroonLightningException):
            LndClient(self.DEV_LND_URL, lnd_invoice_macaroon=self.DEV_LND_INVALID_INVOICE_MACAROON, http_client=self.clear_http_client).create_invoice()

    # ping

    def test_ping(self):
        lightningd_client = LndClient(self.DEV_LND_URL, lnd_invoice_macaroon=self.DEV_LND_INVOICE_MACAROON, http_client=self.clear_http_client)
    ...
    def test_too_big_amount(self):
        with pytest.raises(LightningException):
            lnd_client = LndClient(self.DEV_LND_URL, lnd_invoice_macaroon=self.DEV_LND_INVOICE_MACAROON, http_client=self.clear_http_client)
    ...
    def test_success_invoice_settled(self):
        ...
        lnd_client = StubLndClient(self.DEV_LND_URL, lnd_invoice_macaroon=self.DEV_LND_INVOICE_MACAROON, http_client=self.clear_http_client)
