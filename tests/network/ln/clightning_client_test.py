...
class CLightningClientTest(CypherpunkpayNetworkTestCase):
    ...
    def test_malformed_lightningd_socket_path(self):
        with pytest.raises(LightningException):
            CLightningClient('malformed socket path').create_invoice()

    def test_incorrect_lightningd_socket_path(self):
        with pytest.raises(LightningException):
            CLightningClient('/tmp/nonexisting').create_invoice()

    # ping

    def test_ping(self):
        lightningd_client = CLightningClient(self.DEV_LIGHTNINGD_SOCKET_PATH)
    ...
    def test_success_invoice_settled(self):
        class StubCLightningClient(CLightningClient):
        ...
        lightningd_client = StubCLightningClient(self.DEV_LIGHTNINGD_SOCKET_PATH)
