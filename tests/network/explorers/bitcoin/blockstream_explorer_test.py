...
class BlockstreamExplorerTest(BlockExplorerTest):

    def test_get_height_mainnet_onion(self):
        be = BlockstreamExplorer(self.tor_http_client, btc_network='mainnet', use_tor=True)
        self.assert_btc_mainnet_height(be)

    def test_get_height_mainnet_clear(self):
        be = BlockstreamExplorer(self.tor_http_client, btc_network='mainnet', use_tor=False)
        self.assert_btc_mainnet_height(be)
    ...
    def test_get_height_testnet_clear(self):
        be = BlockstreamExplorer(self.tor_http_client, btc_network='testnet', use_tor=False)
        self.assert_btc_testnet_height(be)
    ...
    def test_non_json_response(self):
        height = self.blockstream_explorer(stub_response='not_json.txt').get_height()
        assert height is None
        height = self.blockstream_explorer(stub_response=None).get_height()
        assert height is None

    def test_no_transactions(self):
        credits = self.blockstream_explorer(
            stub_response='blockstream_txs_bc1quwtrymfzavun8awsv9fqv36wlk0058puuw7mer.json'
        ).get_address_credits(
    ...
    def test_confirmed_skips_unrelated_utxo(self):
        credits = self.blockstream_explorer(
            stub_response='blockstream_txs_3K4J6FSPrpt4uxfhLEqitUeqipA5hR2zJN.json'
        ).get_address_credits(
    ...
    def test_confirmed_large_utxo(self):
        credits = self.blockstream_explorer(
            stub_response='blockstream_txs_1CgA8NopgG1Sg5rrsBkDo8LcT7wT6r7Z3T.json'
        ).get_address_credits(
    ...
    def test_unconfirmed_non_replaceable(self):
        credits = self.blockstream_explorer(
            stub_response='blockstream_txs_bc1qkl6z6c8qz2v2luxv8ajj24t5ymqvex09f667q0.json'
        ).get_address_credits(
    ...
    def test_unconfirmed_replaceable(self):
        ...
        credits = self.blockstream_explorer(
            stub_response='blockstream_txs_bc1q0uwu9upclerpj7hfwtg0kxrucjj5ssq5543mxc.json'
        ).get_address_credits(
    ...
    def blockstream_explorer(self, stub_response: [str, None]):
        return BlockstreamExplorer(http_client=StubHttpClient(stub_response), btc_network='mainnet')
...
class StubHttpClient(DummyHttpClient):
    ...
    def get_text_or_None_on_error(self, url: str, privacy_context: str, verify=None) -> [str, None]:
        if self.response_filename is None:
            return None
        return (this_dir(__file__) / 'test_data' / 'blockstream' / self.response_filename).read_text()
