...
class BitapsExplorerTest(BlockExplorerTest):
    ...
    def test_non_json_response(self):
        height = self.bitaps_explorer(
            stub_response='not_json.txt'
        ).get_height()
        assert height is None
        height = self.bitaps_explorer(stub_response=None).get_height()
        assert height is None

    def test_no_transactions(self):
        credits = self.bitaps_explorer(
            stub_response='bitaps_txs_bc1quwtrymfzavun8awsv9fqv36wlk0058puuw7mer_ifconfirmed.json'
        ).get_address_credits(
    ...
    def test_confirmed_skips_unrelated_utxo(self):
        credits = self.bitaps_explorer(
            stub_response='bitaps_txs_3K4J6FSPrpt4uxfhLEqitUeqipA5hR2zJN_ifconfirmed.json'
        ).get_address_credits(
    ...
    def test_confirmed_large_utxo(self):
        credits = self.bitaps_explorer(
            stub_response='bitaps_txs_1CgA8NopgG1Sg5rrsBkDo8LcT7wT6r7Z3T_ifconfirmed.json'
        ).get_address_credits(
    ...
    def test_unconfirmed_non_replaceable(self):
        credits = self.bitaps_explorer(
            stub_response='bitaps_txs_bc1qkl6z6c8qz2v2luxv8ajj24t5ymqvex09f667q0_ifconfirmed.json'
        ).get_address_credits(
    ...
    def test_unconfirmed_replaceable(self):
        ...
        credits = self.bitaps_explorer(
            stub_response='bitaps_txs_bc1q0uwu9upclerpj7hfwtg0kxrucjj5ssq5543mxc_ifconfirmed.json'
        ).get_address_credits(
    ...
    def bitaps_explorer(self, stub_response: [str, None]):
        return BitapsExplorer(http_client=StubHttpClient(stub_response), btc_network='mainnet')
