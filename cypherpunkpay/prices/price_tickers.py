...
from cypherpunkpay.prices.spotbit_blockchain_commons_source import SpotBitBlockchainCommonsPriceSource
...
class PriceTickers(object):
    ...
    def update_coin(self, coin):
        coin_prices = list(filter(None, [
            ...
            CwatchCoinPriceSource(self._http_client).get(coin, 'usd'),
            SpotBitBlockchainCommonsPriceSource(self._http_client).get(coin, 'usd')
        ]))
