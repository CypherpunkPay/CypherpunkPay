...
class BlockstreamExplorerTest(CypherpunkpayTestCase):
    ...
    def test_non_json_response(self):
        height = self.blockstream_explorer(response='not_json.txt').get_height()
        assert height is None
        height = self.blockstream_explorer(response=None).get_height()
        assert height is None

    def test_no_transactions(self):
        credits = self.blockstream_explorer(
            response='blockstream_txs_bc1quwtrymfzavun8awsv9fqv36wlk0058puuw7mer.json'
        ).get_address_credits(
    ...
    def test_confirmed_skips_unrelated_utxo(self):
        credits = self.blockstream_explorer(
            response='blockstream_txs_3K4J6FSPrpt4uxfhLEqitUeqipA5hR2zJN.json'
        ).get_address_credits(
    ...
    def test_confirmed_large_utxo(self):
        credits = self.blockstream_explorer(
            response='blockstream_txs_1CgA8NopgG1Sg5rrsBkDo8LcT7wT6r7Z3T.json'
        ).get_address_credits(
    ...
    def test_unconfirmed_non_replaceable(self):
        credits = self.blockstream_explorer(
            response='blockstream_txs_bc1qkl6z6c8qz2v2luxv8ajj24t5ymqvex09f667q0.json'
        ).get_address_credits(
    ...
    def test_unconfirmed_replaceable(self):
        ...
        credits = self.blockstream_explorer(
            response='blockstream_txs_bc1q0uwu9upclerpj7hfwtg0kxrucjj5ssq5543mxc.json'
        ).get_address_credits(
    ...
    def blockstream_explorer(self, response: [str, None]):
