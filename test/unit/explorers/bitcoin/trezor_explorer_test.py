...
class TrezorExplorerTest(CypherpunkpayTestCase):
    ...
    def test_empty(self):
        credits = self.mocked_explorer(response='trezor_txs_bc1quwtrymfzavun8awsv9fqv36wlk0058puuw7mer.json').get_address_credits(address='bc1quwtrymfzavun8awsv9fqv36wlk0058puuw7mer', current_height=1)
        self.assertEmpty(credits.any())

    def test_confirmed_skips_unrelated_utxo(self):
        credits = self.mocked_explorer(response='trezor_txs_3K4J6FSPrpt4uxfhLEqitUeqipA5hR2zJN.json').get_address_credits(address='3K4J6FSPrpt4uxfhLEqitUeqipA5hR2zJN', current_height=569992)
    ...
    def test_confirmed_large_utxo(self):
        credits = self.mocked_explorer(response='trezor_txs_1CgA8NopgG1Sg5rrsBkDo8LcT7wT6r7Z3T.json').get_address_credits(address='1CgA8NopgG1Sg5rrsBkDo8LcT7wT6r7Z3T', current_height=1000000)
    ...
    def test_unconfirmed_non_replaceable(self):
        credits = self.mocked_explorer(response='trezor_txs_bc1qkl6z6c8qz2v2luxv8ajj24t5ymqvex09f667q0.json').get_address_credits('bc1qkl6z6c8qz2v2luxv8ajj24t5ymqvex09f667q0', 1)
    ...
    def test_unconfirmed_replaceable(self):
        ...
        credits = self.mocked_explorer(response='trezor_txs_bc1q0uwu9upclerpj7hfwtg0kxrucjj5ssq5543mxc.json').get_address_credits('bc1q0uwu9upclerpj7hfwtg0kxrucjj5ssq5543mxc', 1)
        self.assertEqual(len(credits.any()), 1)
        credit = credits.any()[0]
        self.assertEqual(Decimal('0.06893409'), credit.value())
